...
REM Common LaTeX junk files for the current template and likely future article additions
set CLEAN_FILE_GLOBS=*.acn *.acr *.alg *.aux *.bak *.bcf *.bbl *.blg *.dvi *.fdb_latexmk *.fls *.glg *.glo *.gls *.glsdefs *.idx *.ilg *.ind *.ist *.listing *.loa *.lof *.log *.lol *.lot *.maf *.mtc* *.nav *.nlo *.nls *.nlg *.out *.ps *.pyg *.run.xml *.snm *.spl *.synctex.gz *.tdo *.toc *.xdv *.xml *~ *-blx.bib *SAVE-ERROR
set CLEAN_DIR_GLOBS=tikz-cache _minted-*
...
latexmk -c
for %%f in (%CLEAN_FILE_GLOBS%) do @del /F /Q "%%f" 2>nul
for %%d in (%CLEAN_DIR_GLOBS%) do @if exist "%%d" rmdir /s /q "%%d" 2>nul
